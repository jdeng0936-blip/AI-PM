...
class User(Base):
    ...
    wechat_userid: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)
    # 钉钉身份唯一标识（用于精准推送，可为空）
    ...
    job_title: Mapped[str] = mapped_column(
        String(50), nullable=False, default="", comment="岗位：技术部长/研发工程师/采购经理/仓管 等"
    )
    must_change_password: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, comment="首次登录强制改密"
    )
    ...
    status_until: Mapped[Optional[date]] = mapped_column(Date, nullable=True, comment="状态截止日期（自动恢复 active）")
    # ── 资源负载 ──────────────────────────────────────────────────
    story_points_capacity: Mapped[int] = mapped_column(
        Integer, nullable=False, default=8, comment="Sprint 故事点容量（默认8点/Sprint）"
    )
    ...
    tenant_id: Mapped[str] = mapped_column(
        String(64), nullable=False, default="default", index=True, comment="租户隔离标识"
    )
