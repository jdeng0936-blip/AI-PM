...
async def main():
    ...
    async with async_session() as db:
        ...
        project = (await db.execute(select(Project).join(ProjectMember, ProjectMember.project_id == Project.id).where(ProjectMember.user_id == user.id, Project.is_temporary == True).limit(1))).scalar_one_or_none()
